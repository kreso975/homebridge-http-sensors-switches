 ...
 */
export class platformSmokeSensor {
  public smokeSensorService!: Service;
  ...
  public paramNameSmokeDetected: string = '';
  ...
  public mqttSmokeDetected: string = '';
  ...
  public SmokeStates = {
    SmokeDetected: 0,     // Values: 0 (Inactive), 1 (Active). (On/Off)
  ...
  public SmokeStatusRanges = {
    SmokeDetected: [0, 1],        // Valid values: 0 (Normal), 1 (High Level Detected)
  ...
  constructor(
    ...
    this.paramNameSmokeDetected = device.paramNameSmokeDetected;
    ...
    this.mqttSmokeDetected = device.mqttSmokeDetected;
    ...
    if (this.sharedPolling && this.sharedPollingId) {
      ...
      setInterval(() => {
        ...
        if (data) {
          this.updateSmokeSensorStatusFromSharedData(data);
        }
      }, 10000); // Poll every 10 seconds
    } else if (this.urlStatus) {
      // Fallback to individual polling if shared polling is not enabled
      this.getSmokeState();
      setInterval(this.getSmokeState.bind(this), this.updateInterval);
    }
    ...
    if ( this.deviceType === 'SmokeSensor' && (this.urlStatus || this.mqttBroker)) {
      ...
      if ( this.urlStatus || this.mqttBroker ) {
        // Get the Smoke service if it exists, otherwise create a new Smoke service
        ...
        this.smokeSensorService.setCharacteristic(this.platform.Characteristic.Name, accessory.context.device.deviceName);
        
        if ( this.urlStatus ) {      
          // Register handlers for the characteristics
          this.getStateDefinition().forEach(({ state, param }) => {
            if ( param ) { // Ensure the parameter is valid
              this.smokeSensorService.getCharacteristic(this.platform.Characteristic[state]).on('get', (callback) => {
                callback(null, this.SmokeStates[state]); // Correct state reference
              });
            }
          });
        }
  ...
  private getStateDefinition() {
    return [
      { state: 'SmokeDetected' as const, param: this.paramNameSmokeDetected, topic: this.mqttSmokeDetected, webhook: true, control: 1 },
  ...
  private updateSmokeSensorStatusFromSharedData(data?: Record<string, unknown>): void {
    if (!data) {
      this.platform.log.warn(`${this.deviceName}: No data available for updating Smoke status.`);
      return;
    }
    ...
    this.getStateDefinition().forEach(({ state, param, webhook }) => {
      ...
      const range = this.SmokeStatusRanges[state];
      ...
      if (
        ...
        if (this.enableLogging) {
          if( this.SmokeStates[state] !== value ) {
            this.platform.log.info(`${this.deviceName}: ${state} - [${this.SmokeStates[state]}] SET to: ${value}`);
          }
        }

        this.SmokeStates[state] = value; // Update the state
        this.smokeSensorService.updateCharacteristic(this.platform.Characteristic[state], value); // Update corresponding characteristic
  ...
  private async getSmokeState(): Promise<void> {
    if (!this.urlStatus) {
    ...
    try {
      ...
      this.platform.log.debug(`${this.deviceName}: Smoke states updated to:`, this.SmokeStates);
    } catch (error) {
      const axiosError = error as AxiosError;
      if (axios.isAxiosError(axiosError)) {
        this.platform.log.warn(`${this.deviceName}: Axios error while fetching Smoke state:`, axiosError.message);
      } else {
        this.platform.log.warn(`${this.deviceName}: Unknown error occurred while fetching Smoke state.`);
      }
    }
  }
    ...
    this.mqttClient.on('message', (topic, message) => {
      this.getStateDefinition().forEach(({ state, topic: stateTopic, webhook }) => {
        if (stateTopic === topic) { // Match incoming topic
          ...
          const [min, max] = this.SmokeStatusRanges[state];
          if (min === 0 && max === 1) {
          ...
          // Validate against SmokeStatusRanges
          if (newValue >= min && newValue <= max) {
            this.SmokeStates[state] = newValue; // Update state value
            ...
            this.smokeSensorService.updateCharacteristic(characteristic, newValue);
  ...
  private initDiscordWebhooks(state: keyof typeof this.SmokeStates): void {
    // Prepare a dynamic message including the passed state
    const message = `${this.deviceName}: ${state} - ${this.discordMessage} ${this.getStatus(!!this.SmokeStates[state])}`;
